...
% c_N=2;c_I=2;c_R=1;c_M=3;c_MT=6;
va=0.7;vd=0.7;mu=0.8;k=1;
...
if j==1
   xlswrite('随机为0的数量图.xls',mx);
   xlswrite('随机为0的策略图.xls',mc(:,1:5));
else if j==2
      xlswrite('随机为0.01的数量图.xls',mx);
      xlswrite('随机为0.01的策略图.xls',mc(:,1:5));
    else
           xlswrite('随机为0.06的数量图.xls',mx);
           xlswrite('随机为0.06的策略图.xls',mc(:,1:5));
    end
end
